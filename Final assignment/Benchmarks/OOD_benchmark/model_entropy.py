...
class Model(nn.Module):
    def __init__(self, n_classes=19):
        super().__init__()
        self.segmentor = deeplabv3_resnet50(
            weights= None,
        ...
        self.ood_detector = FM_OODModel()


    def compute_batch_entropy(self, pred: torch.Tensor) -> torch.Tensor:
        """
        Computes the mean entropy for each image in a batch.
        Returns a tensor of shape (batch_size,)
        """
    ...
    def forward(self, x):
        seg_output = self.segmentor(x)['out']

        #ood_score = self.ood_detector(x)
        #threshold = 18.5189 

        ood_score = self.compute_batch_entropy(seg_output)
        threshold = 0.6290
        include_decision = ood_score < threshold

        return seg_output, include_decision
