...
OOD_MODEL_PATH = "/app/ood_model.pt"
...
def main():
    ...
    segmentor_state_dict = torch.load(MODEL_PATH, map_location=device)
    fixed_state_dict = {k.replace('model.', ''): v for k, v in segmentor_state_dict.items()}

    model.segmentor.load_state_dict(fixed_state_dict, strict=False)

    ood_state_dict = torch.load(OOD_MODEL_PATH, map_location=device)
    fixed_state_dict = {k.replace('model.', ''): v for k, v in ood_state_dict.items()}

    model.ood_detector.flow_head.load_state_dict(fixed_state_dict, strict=False)
